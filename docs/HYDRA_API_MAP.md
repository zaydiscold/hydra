...
- `openapi/hydra-api.openapi.json` is the local artifact that agents and scripts inspect.
...
- `openapi/hydra-api.openapi.json` — generated local OpenAPI map of Hydra's Express routes.
...
- `hydra mcp` — starts a private local stdio MCP server for agents.
- `hydra mcp --list-tools` — prints the curated MCP tool list without starting a long-running stdio session.
...
- Use `hydra mcp` for agent-facing fleet tools. It wraps curated CLI commands
  (`status`, `proxy status`, `api-map`, `audit`, and `doctor`) instead of
  generating one raw MCP tool per private Express endpoint.
