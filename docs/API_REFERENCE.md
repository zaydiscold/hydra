...
| `POST` | `/api/accounts/:id/provision` | JWT | Create a management key for one account | Body: `{ keyName? }`. Success: `data`: `{ key, source }` (`source` is e.g. `trpc-cached`, `trpc-…`, `playwright`). **Secret handling:** the server reads the key from **tRPC response JSON** or **Playwright** (`waitForResponse` on `/api/trpc/` + regex / DOM scrape)—**not** from OpenRouter’s “Copy to clipboard” UI and **not** from IDE browser sessions. **422** `PROVISION_FAILED` when the dashboard session is unusable (**HTTP 401/403/423/429** or HTML auth gate), not for every tRPC business error — other tRPC failures still fall through to more procedure candidates and headless Playwright. Playwright / capture failures when no matching **`sk-or-mgmt-…`** appears in the intercepted response or page text surface as **500** with message `Could not extract management key via Playwright`, **`code`:** **`PROVISION_PLAYWRIGHT_EXTRACT`**, and **`hint`** pointing at **`$TMPDIR/hydra-provision-debug/`** (screenshots, `provision-network-*.log`, **`provision-trace-*.zip`** when **`HYDRA_PROVISION_DEBUG=1`**). Env: **`HYDRA_PLAYWRIGHT_HEADED`**, **`HYDRA_PROVISION_DEBUG`**, **`HYDRA_PROVISION_VERBOSE`**, **`HYDRA_PROVISION_NETWORK_LOG`** (see **`.env.example`**). |
