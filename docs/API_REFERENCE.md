...
| `GET` | `/api/accounts/:id/session-status` | JWT | Report whether the stored session is active, expiring, expired, etc. | JSON includes **`status`**, **`sessionExpiry`**, and **`sessionDecryptFailed`** (boolean). **`status`** is one of **`active`**, **`expiring`**, **`expired`**, **`none`**, **`unknown`**, **`error`** (decrypt failure on **`sessionToken`** — see [DASHBOARD_ACCOUNT_STATES.md](DASHBOARD_ACCOUNT_STATES.md)). Does not throw on unreadable session blobs. Successful Clerk logins and refreshes persist a realistic Clerk-session **`sessionExpiry`**. Session-backed actions (**provision**, **redeem**, etc.) run **`ensureSession`** first, which may refresh via **`__client`** when the stored expiry is missing, expired, or close to expiry—see **`ARCHITECTURE_DEEP_DIVE.md`**. |
...
| `POST` | `/api/accounts/:id/management-keys/store` | JWT | Store a provisioned management key | Body: `{ key, source?, keyName? }`. Persists an `sk-or-mgmt-*` key returned from an external provision flow. |
