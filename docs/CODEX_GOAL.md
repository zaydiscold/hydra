...
- **P5:** MCP server wrapping `/api/agent/*` so Claude Code/Cursor get fleet management as native tools
...
- Auth cookie parsing is defensive: malformed percent-encoded cookie values log a redacted warning and flow through the normal 401 path instead of throwing inside middleware; `server/tests/auth-cookie.test.mjs` covers extraction and `requireUnlocked`
...
- A Windows x64 NSIS artifact was produced on 2026-05-18 after staging Playwright's Chrome-for-Testing `145.0.7632.6` Windows payload into an isolated `/private/tmp/hydra-pw-cross/chromium-1208/chrome-win` cache and redirecting Electron/electron-builder caches to `/private/tmp`; `npx electron-builder --win nsis --x64 -c.win.signAndEditExecutable=false` produced `release/Hydra-1.0.0-win-x64.exe`, and `HYDRA_BUILD_TARGET=win32-x64 npm run electron:smoke` passed. Actual Windows install/launch dogfood still requires Windows; this macOS sandbox cannot run Wine/rcedit (`wineserver: Can't check in server_mach_port`) with the normal sign/edit path.
- `scripts/smoke-electron-package.mjs` now requires target-specific Prisma engines and the Windows installer blockmap during package smoke: Windows packages must contain `query_engine-windows.dll.node`, macOS ARM packages must contain the `darwin-arm64` Prisma engine, macOS Intel packages must contain the `darwin` Prisma engine, and `win32-x64` smoke requires `release/Hydra-1.0.0-win-x64.exe.blockmap`. `HYDRA_BUILD_TARGET=win32-x64 npm run electron:smoke`, `HYDRA_BUILD_TARGET=darwin-arm64 npm run electron:smoke`, `npm run test:workflow-contract`, `npm run test:cli`, and `npm run lint` passed on 2026-05-18 17:22-17:26 PDT after this hardening.
- `scripts/package-home-intel.sh` now runs `HYDRA_BUILD_TARGET=darwin-x64 npm run electron:smoke` after the remote Intel build so the x64 package refresh cannot skip the target-specific Chromium, app-shell, and macOS zip artifact checks when `home` is reachable again; `server/tests/workflow-contract.test.mjs` and `hydra audit --json` lock this down
- The macOS Intel artifact was refreshed locally on 2026-05-18 after `ssh home` remained offline: Playwright's Chrome-for-Testing `145.0.7632.6` `chrome-mac-x64` payload was staged into `/private/tmp/hydra-pw-cross/chromium-1208/chrome-mac-x64`, `PLAYWRIGHT_BROWSERS_PATH=/private/tmp/hydra-pw-cross ELECTRON_CACHE=/private/tmp/hydra-electron-cache ELECTRON_BUILDER_CACHE=/private/tmp/hydra-electron-builder-cache npm run electron:build:mac-x64` passed, `HYDRA_BUILD_TARGET=darwin-x64 npm run electron:smoke` passed, `file release/mac/Hydra.app/Contents/MacOS/Hydra` reports `x86_64`, and `codesign --verify --deep --strict --verbose=2 release/mac/Hydra.app` passed. Final Intel GUI dogfood still needs a real Intel Mac or compatible runner, but the x64 artifact is current.
...
- `npm run gate`, full `npm test`, `git diff --check`, and `node bin/hydra.mjs audit --json` passed on 2026-05-18 after the dev-port cleanup hardening, utility/test cleanup visibility pass, package rebuild, native macOS titlebar fix, CLI runtime-diagnostics consistency pass, target-specific Chromium smoke hardening, package-shell smoke hardening, Windows artifact smoke, macOS Intel x64 refresh, final dogfood-runbook wiring, and dependency-audit lockfile fix. Full `npm test` passed again at 19:35 PDT after the direct OpenRouter CLI/API work, log-tail follow hardening, Anime.js `signal` text-effect update, app-shell polish, lazy direct-OpenRouter model-cache loading, and active-scope audit adjustment. `npm run lint`, `npm run build`, `npm run gate`, `HYDRA_BUILD_TARGET=darwin-arm64 npm run electron:smoke`, `HYDRA_BUILD_TARGET=darwin-x64 npm run electron:smoke`, `HYDRA_BUILD_TARGET=win32-x64 npm run electron:smoke`, `npm run test:background-failure-visibility`, `npm run test:cli`, `git diff --check`, and `node bin/hydra.mjs audit --json` also passed at 19:33-19:45 PDT. `hydra audit --json` now separates active code-verifiable evidence from deferred manual GUI/runtime items; packaged GUI dogfood, live MVP dogfood, packaged screenshot audit, and Docker runtime smoke are deferred from active Codex scope instead of blocking source/test/build hardening. Fresh 2026-05-18 probes kept the historical Docker and LaunchServices evidence recorded, but those runtime/manual gaps are not the current Codex plan unless explicitly reopened.
- Docker image construction passed on `ssh home` after moving the builder and runtime images to `node:22-bookworm`; local Docker remains unavailable and the remote Docker Desktop container-start path is blocked, both tracked in `docs/RELEASE_AUDIT.md`
...
- CLI command tests are implemented in `server/tests/cli.test.mjs`; `npm run test:cli` passed with 42 tests on 2026-05-18 19:34 PDT, including the closed-app `hydra audit` evidence checks, guarded redacted metadata import, reversible DB reset, system-command data-dir consistency, packaged Chromium zip doctor detection, status warning-channel, log-tail follow behavior, local `/v1` AI chat, direct OpenRouter-compatible `ai chat --route direct`, `hydra openrouter models/key/credits`, lazy direct-OpenRouter cache writes, and stop timeout/non-JSON source-contract coverage
