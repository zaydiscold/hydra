...
- **500** when no **`sk-or-mgmt-…`** was captured after tRPC + Playwright — response JSON includes **`code`:** **`PROVISION_KEY_NOT_CAPTURED`**, **`legacyCode`:** **`PROVISION_PLAYWRIGHT_EXTRACT`** (for older clients), **`hint`**, **`details`** (redacted: `stage`, `trpcLastError` summary from the HTTP phase, optional dashboard mutation error, `createClicked`, `connectMode` `launch` \| `cdp`, `debugDir`), and **`debugDir`**. Message often still contains `Could not extract management key via Playwright` plus optional last in-browser tRPC error text. Use steps above.
