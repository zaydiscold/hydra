...
- **Playwright Chromium** — runtime code imports `playwright-core`; the full `playwright` package is optional and only needed for installing/finding the browser payload during `electron:prepare`. `scripts/prepare-electron-resources.mjs` locates the Chromium revision, copies only the target platform payload, archives it as `build/electron/chromium.zip`, and ships that archive through `electron-builder` `extraResources`.
...
- **Tray state** — the native tray menu subscribes to the shared proxy gate and updates between `proxy running` and `proxy disabled` when the renderer toggles `/v1` routing.
...
- **Splash sequence** — boot keeps the serialized splash-to-main handoff, but the old indeterminate pulsing/sweep indicator is now a deterministic progress bar that fills across the full visible splash interval. Minimum visible duration was bumped from 6.8 s → **7 s** on 2026-05-06; the bar's `fillbar` keyframe duration in `electron/app/windows.js` was bumped in lockstep so the bar reaches 100 % exactly as the splash dismisses.
...
| 9 | macOS zip/DMG, Windows NSIS, and Linux packages | macOS zip verified locally; DMG requires unsandboxed macOS `hdiutil`; verify each target before release |
| 10 | CI coverage for build and smoke path | GitHub Actions workflows cover lint/test/gate plus macOS, Windows, and Linux package smoke; each platform still needs real launch dogfood before release |
...
| `docs/_archive/historical/ELECTRON_MASTER_PLAN.md` | Historical architecture decisions and original plan |
