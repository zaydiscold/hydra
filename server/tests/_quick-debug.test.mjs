/**
 * Quick debug: replicate launch-compat test pattern
 */
...
console.log('Initial server:', server === null ? 'null' : server);

test('start test', async () => {
  await bootstrap();
  console.log('After bootstrap, server:', server === null ? 'null' : typeof server);
  const addr = server?.address();
  console.log('Address:', addr);
  assert.ok(addr, 'should have address');
  console.log('Port:', addr.port);
  assert.ok(addr.port > 0, 'port should be > 0');
});
