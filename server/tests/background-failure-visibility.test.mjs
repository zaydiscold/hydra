...
test('account generator browser signup uses the encrypted proxy pool when present', () => {
  ...
  assert.match(source, /!termsAccepted && postPasswordCheckpoint\.legalBlocked/);
  assert.match(source, /fillVisibleSignupPassword\(page, task\.metadata\.password, task\.taskId\)/);
  assert.match(source, /acceptVisibleSignupTerms\(page, task\.taskId\)/);
