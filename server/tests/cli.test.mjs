...
test('hydra audit reports release evidence and deferred manual items without launching the app', () => {
  ...
  assert.ok(report.items.some((item) => item.id === 'docker-runtime' && item.state === 'ok' && /Runtime Smoke/.test(item.evidence) && /newest recorded checkpoint run 26724119196/.test(item.evidence)));
