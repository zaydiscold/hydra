...
test('heavy ambient chrome settles while lightweight space accents keep moving', () => {
  const app = readRepoFile('src/App.jsx');
  const css = readRepoFile('src/index.css');
  ...
  assert.doesNotMatch(app, /useVisibleRecurringTask\('App\.settledMeteorLoop'/);
  assert.doesNotMatch(app, /setTrackedTimeout\('App\.settledMeteorWindow'/);
  assert.match(app, /const \[planetMoonStep, setPlanetMoonStep\] = useState\(0\)/);
  assert.match(app, /useVisibleRecurringTask\('App\.planetMoonStep', advancePlanetMoons, 10800, \{ enabled: !ambientMotion \}\)/);
  assert.match(app, /'--moon-phase-primary': `\$\{\(planetMoonPhase \* 18\) % 360\}deg`/);
  assert.match(app, /const \[secretMeteorMode, setSecretMeteorMode\] = useState\(false\)/);
  assert.match(app, /const meteorVolleyComboRef = useRef\(0\)/);
  assert.match(app, /const meteorVolleyLastAtRef = useRef\(0\)/);
  assert.match(app, /Math\.min\(4, meteorVolleyComboRef\.current \+ 1\)/);
  assert.match(app, /setTrackedTimeout\('App\.secretMeteorVolley'/);
  assert.match(app, /event\.key\?\.toLowerCase\(\) !== 'p'/);
  assert.match(app, /target\?\.isContentEditable \|\| \['input', 'textarea', 'select'\]\.includes\(tagName\)/);
  assert.match(app, /Secret meteor control unlocked\. Press P to call strikes\./);
  assert.match(app, /<div className="meteor-container" key=\{meteorVolley\.count\}>/);
  ...
  assert.match(app, /className="secret-meteor-control"/);
  assert.match(app, /<kbd>P<\/kbd>/);
  ...
  assert.match(app, /app-shell--secret-meteor-mode/);
  assert.match(app, /app-shell--meteor-volley-\$\{meteorVolley\.variant\}/);
  assert.match(app, /app-shell--meteor-volley-intensity-\$\{meteorVolley\.intensity\}/);
  assert.doesNotMatch(app, /app-shell--meteor-burst/);
  assert.doesNotMatch(app, /app-shell--easter-mode/);
  assert.match(app, /<button type="button" className="planet planet-1" style=\{planetMoonStyle\} onClick=\{handleSecretMeteorUnlockClick\}/);
  ...
  assert.match(css, /\.app-shell--ambient-motion \.meteor\s*\{[\s\S]*?animation:\s*meteorFall var\(--meteor-duration, 8s\) linear infinite;/);
  ...
  assert.doesNotMatch(css, /app-shell--meteor-burst/);
  assert.match(cssRuleBlock(css, '.meteor {'), /height:\s*calc\(var\(--meteor-length, 156px\) \* var\(--meteor-tail-scale, 1\)\)/);
  assert.match(css, /\.app-shell--motion-settled\.app-shell--meteor-volley\.app-shell--meteor-volley-a \.meteor\s*\{[\s\S]*?animation:\s*meteorVolleyA var\(--meteor-volley-duration, 1\.46s\)/);
  assert.match(css, /\.app-shell--motion-settled\.app-shell--meteor-volley\.app-shell--meteor-volley-b \.meteor\s*\{[\s\S]*?animation:\s*meteorVolleyB var\(--meteor-volley-duration, 1\.46s\)/);
  assert.match(css, /\.app-shell--motion-settled\.app-shell--meteor-volley-intensity-2 \.meteor\s*\{[\s\S]*?--meteor-tail-scale:\s*1\.22;/);
  assert.match(css, /\.app-shell--motion-settled\.app-shell--meteor-volley-intensity-3 \.meteor,[\s\S]*?\.app-shell--motion-settled\.app-shell--meteor-volley-intensity-4 \.meteor\s*\{[\s\S]*?--meteor-tail-scale:\s*1\.38;[\s\S]*?--meteor-volley-duration:\s*1\.14s;/);
  assert.match(css, /\.app-shell--motion-settled\.app-shell--meteor-volley-intensity-3 \.meteor-container,[\s\S]*?\.app-shell--motion-settled\.app-shell--meteor-volley-intensity-4 \.meteor-container\s*\{[\s\S]*?animation:\s*meteorVolleyShake 360ms/);
  assert.match(css, /\.secret-meteor-control\s*\{/);
  assert.match(css, /@keyframes meteorVolleyShake/);
  assert.doesNotMatch(css, /\.easter-meteor-control\s*\{/);
  assert.match(css, /\.meteor:nth-child\(1\)\s*\{[\s\S]*?--meteor-burst-duration:\s*0\.95s;[\s\S]*?--meteor-volley-delay:\s*0ms;/);
  assert.match(css, /\.meteor:nth-child\(3\)\s*\{[\s\S]*?--meteor-burst-duration:\s*1\.02s;[\s\S]*?--meteor-volley-delay:\s*160ms;/);
  assert.match(css, /\.meteor:nth-child\(5\)\s*\{[\s\S]*?--meteor-burst-duration:\s*0\.9s;[\s\S]*?--meteor-volley-delay:\s*320ms;/);
  assert.doesNotMatch(css, /meteorPulseFall/);
  assert.doesNotMatch(css, /meteorBurstFall/);
  ...
  assert.match(cssRuleBlock(css, '.meteor {'), /rotate\(var\(--meteor-angle, 45deg\)\)/);
  assert.match(cssRuleBlock(css, '@keyframes meteorFall {'), /rotate\(var\(--meteor-angle, 45deg\)\) translate3d\(0, var\(--meteor-distance, 280vh\), 0\)/);
  assert.doesNotMatch(cssRuleBlock(css, '@keyframes meteorFall {'), /translate3d\(var\(--meteor-dx/);
...
test('1.5 desktop operator polish stays wired to real controls', () => {
  ...
  assert.doesNotMatch(cssRuleBlock(css, '.planet-moon {'), /animation:/);
  assert.match(cssRuleBlock(css, '.planet-moon {'), /transform:\s*translate3d\(-50%, -50%, 0\) rotate\(calc\(var\(--moon-orbit-start\) \+ var\(--moon-phase\)\)\) translateX\(var\(--moon-radius\)\)/);
  assert.match(cssRuleBlock(css, '.planet-moon {'), /transition:\s*transform 420ms linear;/);
  assert.match(css, /\.app-shell--ambient-motion \.planet-moon\s*\{[\s\S]*?animation:\s*moonOrbitLinear var\(--moon-orbit-duration\) linear infinite;/);
  assert.match(css, /\.planet-moon\s*\{[\s\S]*?will-change:\s*auto;/);
  assert.match(css, /--moon-phase:\s*var\(--moon-phase-primary, 0deg\);/);
  assert.doesNotMatch(css, /steps\(var\(--moon-orbit-steps\)/);
  ...
  assert.match(css, /\.planet-moon-orbit\s*\{[\s\S]*?--moon-orbit-duration:\s*18\.4s;[\s\S]*?--moon-orbit-squash:\s*0\.82;/);
  assert.match(css, /\.planet-moon-orbit--inner\s*\{[\s\S]*?--moon-orbit-duration:\s*13\.2s;[\s\S]*?--moon-radius:\s*112px;/);
  assert.match(css, /\.planet-moon-orbit--outer\s*\{[\s\S]*?--moon-orbit-duration:\s*30\.5s;[\s\S]*?--moon-radius:\s*186px;/);
