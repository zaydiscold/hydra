...
test('heavy ambient chrome settles while lightweight space accents keep moving', () => {
  ...
  assert.match(css, /\.app-shell--motion-settled\.app-shell--meteor-pulse \.meteor:nth-child\(-n\+5\)\s*\{[\s\S]*?animation:\s*meteorPulseFall var\(--meteor-pulse-duration, 2\.4s\) linear 1;/);
  assert.match(css, /@keyframes meteorPulseFall\s*\{[\s\S]*?translate3d\(var\(--meteor-dx, -122vw\), var\(--meteor-dy, 46vh\), 0\)/);
  ...
  assert.match(cssRuleBlock(css, '@keyframes meteorFall {'), /translate3d\(var\(--meteor-dx, -122vw\), var\(--meteor-dy, 46vh\), 0\)/);
