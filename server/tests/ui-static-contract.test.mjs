...
test('heavy ambient chrome settles while lightweight space accents keep moving', () => {
  ...
  assert.match(app, /useVisibleRecurringTask\('App\.planetMoonStep', advancePlanetMoons, 2800, \{ enabled: !ambientMotion \}\)/);
  ...
  assert.match(app, /<button type="button" className="planet planet-1" style=\{planetMoonStyle\} onClick=\{handleEasterTriggerClick\}/);
...
test('1.5 desktop operator polish stays wired to real controls', () => {
  ...
  assert.match(cssRuleBlock(css, '.planet-moon {'), /transition:\s*transform 850ms linear;/);
