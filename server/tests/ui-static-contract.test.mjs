...
test('ambient app chrome animations settle after launch instead of running forever', () => {
  ...
  assert.match(app, /const spaceMotionPhaseRef = useRef\(\{ primary: -18, inner: 42, outer: -132 \}\)/);
  ...
  assert.match(css, /\.app-shell--motion-settled \.meteor\s*\{[\s\S]*?animation:\s*none;/);
  assert.match(css, /\.app-shell--motion-settled \.meteor\s*\{[\s\S]*?opacity:\s*0;/);
  ...
  assert.match(cssRuleBlock(css, '@keyframes meteorFall {'), /translate3d\(var\(--meteor-dx, -112vw\), var\(--meteor-dy, 46vh\), 0\)/);
  assert.match(cssRuleBlock(css, '@keyframes meteorPulseFall {'), /translate3d\(var\(--meteor-dx, -112vw\), var\(--meteor-dy, 46vh\), 0\)/);
