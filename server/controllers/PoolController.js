...
class PoolController extends BaseController {
  ...
  async getPoolData(req, res) {
    ...
    try {
      ...
      // ⚡ Bolt: Batch fetch all local keys by account to avoid N+1 queries in the loop below
      const allLocalKeysMap = await store.getAllLocalKeysByAccount(req.user.id);
      ...
      const accountResults = await Promise.allSettled(
        allAccounts.map(async (account) => {
          ...
          const localKeys = allLocalKeysMap.get(account.id) || [];
