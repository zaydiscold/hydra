/**
 * Hydra Electron — Window Management
 *
 * Splash window + main BrowserWindow creation and focus helpers.
 */
...
export function createSplashWindow() {
  ...
  splashWindow.once('closed', () => {
    // Caller holds the splashWindow reference; this just lets GC collect it.
  });
  return splashWindow;
}

/** Show + focus the main window, restoring the dock if it was hidden. */
export function showAndFocusMainWindow(mainWindow) {
 ...
 * @param {string|null} opts.windowURL - URL of the Hydra UI
 * @param {Function} opts.openExternalUrl - async function to open external URLs
 * @param {Function} [opts.onClose] - async (event, win) => void; called when window close is requested
 * @returns {BrowserWindow}
 */
export function createMainWindow({ preloadPath, windowURL, openExternalUrl, onClose, show = false } = {}) {
